...
const HOURS_PER_WORKDAY = 8; // matches the "workdays (8h)" convention used by prediction/resolution-history features

/** Formats a duration in 8-hour workdays (e.g. from predictResolutionDays) as "1d 2h 30m" — never as calendar hours. */
export function formatWorkdayDuration(days: number): string {
  const totalMinutes = Math.round(days * HOURS_PER_WORKDAY * MINUTES_PER_HOUR);
  if (totalMinutes <= 0) return "0m";

  const d = Math.floor(totalMinutes / (HOURS_PER_WORKDAY * MINUTES_PER_HOUR));
  const remAfterDays = totalMinutes % (HOURS_PER_WORKDAY * MINUTES_PER_HOUR);
